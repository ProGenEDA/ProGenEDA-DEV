# Final JSON To KiCad Combination Project Run
...
Each project also has a two-artifact output boundary under `outputs/<circuit_id>/`: `user_project/PROGEN_KICAD_PROJECT.zip` is the only user-downloadable export, while `internal/internal_bundle.zip` is backend-only metadata containing the main input JSON, all generated stage JSON, validation reports, and retained arrangement variants with the accepted variant marked.
