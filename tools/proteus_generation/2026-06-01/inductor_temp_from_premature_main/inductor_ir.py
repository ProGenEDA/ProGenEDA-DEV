"""CircuitIR parser for the locked inductor generator."""
...
from .resistor_ir import ComponentPosition, VisualWire
...
INDUCTOR_VALUE_RE = re.compile(r"^[0-9]+(?:\.[0-9]+)?[A-Za-z]{1,3}$")
...
@dataclass(frozen=True)
class InductorCircuitIR:
    schema_version: str
    generator_target: str
    project: InductorProject
    nodes: tuple[InductorNode, ...]
    components: tuple[InductorComponent, ...]
    layout: InductorLayout
    metadata: dict[str, Any]


@dataclass(frozen=True)
class InductorValidationReport:
    errors: tuple[Issue, ...]
    warnings: tuple[Issue, ...] = ()
    circuit: InductorCircuitIR | None = None
...
def visible_inductor_value(value: str, visual: dict[str, Any] | None = None) -> str:
    override = (visual or {}).get("visible_value")
    if isinstance(override, str) and override.isascii() and len(override) in (3, 4):
        return override
    if value.isascii() and len(value) in (3, 4):
        return value
    raise ValueError("Inductor visible values currently require a 3- or 4-character ASCII value or visual.visible_value override.")


def parse_inductor_ir(payload: Any) -> tuple[InductorCircuitIR | None, list[Issue]]:
    issues: list[Issue] = []
    ...
    components: list[InductorComponent] = []
...
def validate_inductor_circuit(ir: InductorCircuitIR) -> InductorValidationReport:
    errors: list[Issue] = []
    warnings: list[Issue] = []
    ...
    if not refs:
        errors.append(Issue("MISSING_COMPONENTS", "At least one inductor is required.", "$.components"))
    if len(refs) > 3:
        errors.append(Issue("TOO_MANY_INDUCTORS", "The locked inductor generator currently supports at most three inductors.", "$.components"))
    ...
    uses_power_ground = any(endpoint in power_nodes or endpoint in ground_nodes for component in ir.components for endpoint in component.nodes)
    ...
    for index, component in enumerate(ir.components):
        ...
        if component.type != "INDUCTOR":
            errors.append(Issue("UNSUPPORTED_COMPONENT_TYPE", "Use INDUCTOR.", f"{path}.type"))
        if not INDUCTOR_VALUE_RE.match(component.value):
            errors.append(Issue("INVALID_INDUCTOR_VALUE", "Use compact inductor values such as `1mH` or `10uH`.", f"{path}.value"))
        else:
            try:
                visible = visible_inductor_value(component.value, component.visual)
                expected_len = 4 if index == 2 else 3
                if len(visible) != expected_len and not uses_power_ground:
                    errors.append(
                        Issue(
                            "UNSUPPORTED_VISIBLE_VALUE_SLOT",
                            f"Inductor slot {index + 1} currently requires a {expected_len}-character visible value.",
                            f"{path}.visual.visible_value",
                        )
                    )
        ...
        if left in ground_nodes:
            errors.append(Issue("GROUND_LEFT_ENDPOINT_UNSUPPORTED", "The locked inductor method only supports ground on right endpoints.", f"{path}.nodes[0]"))
        if right in power_nodes:
            errors.append(Issue("POWER_RIGHT_ENDPOINT_UNSUPPORTED", "The locked inductor method only supports power on left endpoints.", f"{path}.nodes[1]"))

    if uses_power_ground:
        if len(ir.components) != 1:
            errors.append(
                Issue(
                    "INDUCTOR_POWER_GROUND_MULTI_UNVALIDATED",
                    "Power/ground inductor generation is locked only for one V0-to-G0 inductor using donor04 order.",
                    "$.components",
                )
            )
        elif ir.components[0].nodes != ("V0", "G0"):
            errors.append(
                Issue(
                    "INDUCTOR_POWER_GROUND_TOPOLOGY_UNVALIDATED",
                    "The accepted power/ground inductor topology is exactly component.nodes = [`V0`, `G0`].",
                    "$.components[0].nodes",
                )
            )
    ...
    if ir.layout.visual_wires:
        warnings.append(Issue("VISUAL_WIRES_SKIPPED", "Standalone visual wires are parsed but not emitted in the locked inductor generator.", "$.layout.visual_wires"))
    ...
    return InductorValidationReport(errors=tuple(errors), warnings=tuple(warnings), circuit=ir)


def validate_inductor_payload(payload: Any) -> InductorValidationReport:
