"""Generate probe circuits for the 2026-06-18 new-component mega donors.

This is an experimental, no-terminal component-placement pack. It extends the
accepted mega separation method to donor records whose component IDs are not all
ordinary R/U/Q/D refs. In particular, Proteus stores the switch records in this
donor as anonymous component records, so the parser must treat anonymous
COMPONENT ID records as first-class packet starts.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v1_temp_2026_06_18"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V1_TEMP_2026_06_18.zip"
...
def build_cases() -> dict[str, object]:
    ...
    for index, family in enumerate(new_families, start=1):
        cases.append(
            write_case(
                f"S{index:02d}_{family.replace('-', '_')}_SINGLE",
                single,
                select_groups(single, {family: 1}),
                f"Single {family} packet selected from the new-component single mega donor.",
                extra={"requested_counts": {family: 1}},
            )
        )

    multiple_specs = [
        ("M01_NEW_FAMILIES_ONE_EACH", {family: 1 for family in new_families}, "One packet of every newly observed family."),
        ("M02_NEW_FAMILIES_THREE_EACH", {family: 3 for family in new_families}, "Three packets of every newly observed family."),
        ("M03_SWITCH_FUSE_BRIDGE_POWER_PATH", {"SWITCH": 5, "FUSE": 5, "BRIDGE": 5, "4007": 5, "LED-RED": 5}, "Power-path style group: switches, fuses, bridge rectifiers, 4007, LEDs."),
        ...
        ("M05_TRANSISTOR_DIODE_VARIANTS", {"2N3904": 5, "2N4401": 5, "1N4733A": 5, "LED-RED": 5, "FUSE": 3}, "Discrete transistor and diode variants plus fuses."),
        (
            ...
            "New families mixed with finalizable resistor/capacitor and source packets from the same donor.",
        ),
        (
    ...
    for case_id, counts, description in multiple_specs:
        state = five if any(count > len(single.groups_by_family.get(family, ())) for family, count in counts.items()) else single
        cases.append(write_case(case_id, state, select_groups(state, counts), description, extra={"requested_counts": counts}))

    return {
        "experiment": "new_component_mega_probe_v1_temp_2026_06_18",
        "purpose": "Probe no-terminal packet separation for new families from the source/semi mega donors without requiring solo donors.",
