"""Generate V4 focused repair probes for updated new-component mega failures.
...
V3 proved that pruning ROOT.CDB for this donor family is unsafe: user feedback
reported every V3 case crashed. V4 therefore keeps the V2 full-donor ROOT.CDB
policy and tests only safer packet selection.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v4_fullcdb_focus_temp_2026_06_18"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V4_FULLCDB_FOCUS_TEMP_2026_06_18.zip"
...
def build_cases() -> dict[str, object]:
    ...
    for count in (23,):
        focused_specs.extend(
            [
                (
                    "F23A_FUSE_23X_SKIP_FIRST_FULLCDB",
                    safe_select("FUSE", count),
                    "23 fuse packets, skipping the first anonymous fuse packet; full donor CDB retained.",
                    {"requested_counts": {"FUSE": count}, "selector": "anonymous_offset_1"},
                ),
                (
                    "F23B_FUSE_23X_OFFSET_21_FULLCDB",
                    select_family_window(five, "FUSE", count, offset=21),
                    "23 fuse packets starting at the next visible donor block; full donor CDB retained.",
    ...
    for count in (1, 5, 15, 23):
        focused_specs.append(
            (
                f"P{count:02d}_PNP_{count:02d}X_CDB_ZERO_TAIL_FULLCDB",
    ...
    for count in (5, 23):
        focused_specs.append(
            (
                f"H{count:02d}_POT_HG_{count:02d}X_SKIP_FIRST_FULLCDB",
                select_family_window(five, "POT-HG", count, offset=1),
                f"{count} POT-HG packets skipping the first control-like packet; full donor CDB retained.",
    ...
    return {
        ...
        "purpose": "Focused repair probes for V2 user failures using the updated new-component mega donor as the main source, CDB-backed natural-ref selection, and full donor ROOT.CDB after V3 pruned-CDB rejection.",
