"""Generate V10 manual-S04-derived control probes for SWITCH/POT-HG.

V9 user result:
- P02/P04 fixed POT-HG but showed SWITCH issue;
- all S cases fixed POT-HG but showed SWITCH issue;
- manual-fixed S04 advanced the SWITCH packet window by one more record and
  Proteus saved a 00 00 chunk prefix plus a pruned CDB.

Rules under test:
- keep the V6 accepted 00 08 chunk header;
- keep accepted FUSE/PNP/VPULSE selectors and full donor ROOT.CDB;
- test SWITCH offset 23 against the known-good POT-HG windows.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v10_manual_s04_temp_2026_06_19"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V10_MANUAL_S04_TEMP_2026_06_19.zip"
...
def build_cases() -> dict[str, object]:
    ...
    profile_specs = [
        (
            "T01_ONE_SW23_POT40_P08_FULL",
            "M01_CONTROL_ONE_EACH",
            {"SWITCH": 23, "POT-HG": 40},
            {},
            "M01: use V9 known-good POT-HG block3 and manual-S04-derived SWITCH offset 23.",
            b"\x00\x08",
            False,
        ),
        (
            "T02_ONE_SW23_POT40_P00_FULL",
            "M01_CONTROL_ONE_EACH",
            {"SWITCH": 23, "POT-HG": 40},
            {},
            "M01: same as T01 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
        ),
        (
            "T03_FIVE_SW23_POT40_P08_FULL",
            "M02_CONTROL_FIVE_EACH",
            {"SWITCH": 23, "POT-HG": 40},
            {},
            "M02: use V9 known-good POT-HG block3 and manual-S04-derived SWITCH offset 23.",
            b"\x00\x08",
            False,
        ),
        (
            "T04_FIVE_SW23_POT40_P00_FULL",
            "M02_CONTROL_FIVE_EACH",
            {"SWITCH": 23, "POT-HG": 40},
            {},
            "M02: same as T03 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
        ),
        (
            "T05_LARGE_SW23_POT20_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            {"SWITCH": 23, "POT-HG": 20},
            {},
            "M07: use known-good large POT-HG block2 and manual-S04-derived SWITCH offset 23.",
            b"\x00\x08",
            False,
        ),
        (
            "T06_LARGE_SW23_POT20_P00_FULL",
            "M07_CONTROL_LARGE_STRESS",
            {"SWITCH": 23, "POT-HG": 20},
            {},
            "M07: same as T05 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
        ),
        (
            "T07_LARGE_SW23_POT20_P08_PRUNED",
            "M07_CONTROL_LARGE_STRESS",
            {"SWITCH": 23, "POT-HG": 20},
            {},
            "M07: same as T05 but prune CDB to selected package refs like Proteus manual save.",
            b"\x00\x08",
            True,
        ),
        (
            "T08_LARGE_SW23_POT20_P00_PRUNED",
            "M07_CONTROL_LARGE_STRESS",
            {"SWITCH": 23, "POT-HG": 20},
            {},
            "M07: closest generated match to manual-fixed S04: SWITCH offset 23, POT-HG offset 20, prefix 00 00, pruned CDB.",
            b"\x00\x00",
            True,
        ),
    ]
    for case_id, base_case, offsets, skip_each, description, chunk_prefix, prune_cdb in profile_specs:
        counts = counts_by_base[base_case]
        focused_specs.append(
            (
                case_id,
                control_profile_select_many(
                    counts,
                    switch_offset=offsets.get("SWITCH"),
                    pothg_offset=offsets.get("POT-HG"),
                    switch_skip_each_block=skip_each.get("SWITCH"),
                    pothg_skip_each_block=skip_each.get("POT-HG"),
                ),
                description,
                {
                    "requested_counts": counts,
                    "selector": "v10_manual_s04_derived_switch_offset23",
    ...
    return {
        "experiment": "new_component_mega_probe_v10_manual_s04_temp_2026_06_19",
        "purpose": "Mixed-case correction using the user-provided manual-fixed S04 as byte-level evidence.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "Use SWITCH offset 23 with known-good POT-HG windows; test 00 08 vs 00 00 prefix and full vs pruned CDB policies.",
        "donor_counts": {
