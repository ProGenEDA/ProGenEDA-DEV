...
def _ensure_unique_final_wire_suffixes(
    ...
    *,
    ...
    for iteration in range(1, 4097):
        ...
        # Rebuild the ROOT.DSN frame in memory for the next address scan.  The
        # prior implementation zip-wrote and re-read the full project for each
        # one-byte label adjustment; a 20x mix can need several independent
        # collision fixes, turning a deterministic eight-step calculation into
        # a minute-long I/O loop. The final rebased DSN is still written once
        # and read back below before being reported as valid.
