...
OUT=ROOT/'R21_E001_B02_V9_NO_PREMATURE_TERMINATORS'
ZIP=ROOT/'R21_E001_B02_V9_NO_PREMATURE_TERMINATORS.zip'
...
def suffix_for(idx:int, kind:str) -> int:
    # Donor pattern: input 0159,0317,04d5,0693 ; output 018b,0349,0507,06c5.
    # Difference per component = 0x01be. Generate unique suffixes for all 21.
    base=0x0159 if kind=='in' else 0x018b
    return (base+(idx-1)*0x01be)&0xffff

def patch_input(t:bytearray,label:str,sx:int,sy:int,lx:int,ly:int,idx:int):
    ...
    s=suffix_for(idx,'in')
    r[-4:-2]=u16(s); r[-2]=0x01; r[-1]=0x00
    return bytes(r),s

def patch_output(t:bytearray,label:str,sx:int,sy:int,lx:int,ly:int,idx:int):
    r=bytearray(t); lab=label.encode('ascii'); assert len(lab)==2
    r[1:5]=i32(sx); r[5:9]=i32(sy)
    r[31]=2; r[32:34]=lab; r[34:38]=i32(lx); r[38:42]=i32(ly)
    s=suffix_for(idx,'out')
    r[-4:-2]=u16(s); r[-2]=0x01; r[-1]=0x00
    return bytes(r),s

def patch_res(t:bytearray,idx:int,ref:str,val:str,x:int,y:int,in_suf:int,out_suf:int):
    r=bytearray(t); assert len(ref)==2 and len(val)==2
    # label/value text and coordinates from the known B02 resistor visible record family
    ...
    r[324:328]=u32(idx)
    # CRITICAL V8 FIX: resistor visual record carries the paired terminal suffixes near its end.
    # Previous packs changed terminal suffixes but left these as donor values, causing mismatched object links.
    r[337:339]=u16(in_suf);  r[339]=0x01; r[340]=0x00
    r[341:343]=u16(out_suf); r[343]=0x01; r[344]=0x00
    # V9 FIX: clear donor record terminator. Only the very final object in the whole chunk may end with FF.
    r[-1]=0x00
    return bytes(r)
...
def patch_wire(t:bytearray,x1:int,y1:int,x2:int,y2:int):
    ...
    # V9 FIX: clear donor record terminator. Cycling donor group #4 used to carry an FF here,
    # causing Proteus to stop after R4/R8/... even though more groups followed.
    r[-1]=0x00
    return bytes(r)
...
def build_object_chunk(n:int):
    ...
    for idx,left,right,col,row in conns:
        ...
        inp,in_suf=patch_input(IN_TS[(idx-1)%4],left,in_sym_x,y,in_label_x,y,idx)
        outp,out_suf=patch_output(OUT_TS[(idx-1)%4],right,out_sym_x,y,out_label_x,y,idx)
        inputs.append(inp); outputs.append(outp)
        res,w1,w2=GROUPS[(idx-1)%4]
        groups.append(patch_res(res,idx,REFS[idx-1],VIS_VALUES[idx-1],x,y,in_suf,out_suf))
        groups.append(patch_wire(w1,in_tip_x,y,left_pin_x,y))
        groups.append(patch_wire(w2,out_tip_x,y,right_pin_x,y))
        maps.append({'idx':idx,'ref':REFS[idx-1],'value':VALUES[idx-1],'visible':VIS_VALUES[idx-1],'left':left,'right':right,'in_suffix':f'{in_suf:04x}','out_suffix':f'{out_suf:04x}','x':x,'y':y})
...
def extract_chunk(dsn:bytes):
    fi=dsn.find(b'ISIS CIRCUIT FILE'); obj=dsn.find(b'OBJECT DATA',fi); sec=dsn.find(b'ISIS CIRCUIT FILE',fi+1)
    return dsn[obj+len(b'OBJECT DATA'):sec]

def validate_chunk(c:bytes,n:int,maps:list):
    issues=[]
    expected_len=1+n*103+n*104+1+n*(346+50+50)
    if len(c)!=expected_len: issues.append(f'chunk length {len(c)} != {expected_len}')
    if c[0]!=0: issues.append('chunk header not 00')
    if c[-1]!=0xff: issues.append('final chunk byte not FF')
    if c.count(b'$TERINPUT')!=n: issues.append('bad input terminal count')
    if c.count(b'$TEROUTPUT')!=n: issues.append('bad output terminal count')
    if c.count(b'COMPONENT ID')!=n: issues.append('bad component id count')
    if c.count(b'WIRE')!=2*n: issues.append('bad wire count')
    # V9 boundary validation: record-final bytes for resistor/wire group objects must be 00 except final object.
    base=1+n*103+n*104+1
    for j in range(n):
        g=base+j*(346+50+50)
        expected_res=0
        expected_w1=0
        expected_w2=0xff if j==n-1 else 0
        if c[g+345]!=expected_res: issues.append(f'group {j+1} resistor terminator {c[g+345]:02x}')
        if c[g+346+49]!=expected_w1: issues.append(f'group {j+1} wire1 terminator {c[g+346+49]:02x}')
        if c[g+396+49]!=expected_w2: issues.append(f'group {j+1} wire2 terminator {c[g+396+49]:02x}')
    # Verify every generated terminal suffix appears again in its corresponding resistor record.
    for m in maps:
        inb=struct.pack('<H', int(m['in_suffix'], 16))
        outb=struct.pack('<H', int(m['out_suffix'], 16))
        if c.count(inb)<2: issues.append(f"input suffix {m['in_suffix']} appears {c.count(inb)} times")
        if c.count(outb)<2: issues.append(f"output suffix {m['out_suffix']} appears {c.count(outb)} times")
    return issues
...
def main():
    ...
    manifest={}
    for n,name in [(1,'TEST_B02_LINKED_R1_TERMINAL_RESISTOR_TERMINAL'),(2,'TEST_B02_LINKED_R2_SERIES_TERMINALS'),(21,'FINAL_B02_LINKED_R21_7PAR7_PLUS_7SERIES_TERMINALS')]:
    ...
    print((OUT/'manifest.json').read_text()[:2000])
