...
def _remove_catalogue_leading_component_text_records(
    data: bytes,
    geometry: dict[str, Any],
    *,
    family: str,
    ...
    """Remove one exact donor-proven leading component text record.

    A locked component-placer packet can retain an initial ``COMPONENT ID``
    annotation that an accepted terminalized donor does not have. The catalogue
    must name the field and first following component marker; the current packet must
    start with its current reference, and the expected ``Default Font`` header
    must occur exactly once before that marker. Any mismatch is rejected rather
    than guessing at a packet boundary.
    """

    raw_rules = geometry.get("leading_component_text_record_removals", ())
    if raw_rules in (None, ()):
        return data, []
    if not isinstance(raw_rules, (list, tuple)):
        raise ValueError(
            f"{family} {key} leading_component_text_record_removals must be a list."
        )
    ...
    try:
        ...
        raise ValueError(
            f"{family} {key} has a non-ASCII component key for text normalization."
        ) from exc
    if not key_bytes or len(key_bytes) > 255:
        raise ValueError(
            f"{family} {key} has an unsupported component-key length for text normalization."
        )
    reference_marker = b"\xff" + bytes((len(key_bytes),)) + key_bytes
    ...
    for raw_rule in raw_rules:
        if not isinstance(raw_rule, dict):
            raise ValueError(
                f"{family} {key} has a malformed leading component-text rule."
            )
        raw_field = raw_rule.get("field")
        raw_next_marker = raw_rule.get("next_record_marker_ascii")
        if not isinstance(raw_field, str) or not raw_field:
            raise ValueError(
                f"{family} {key} leading component-text rule needs a nonempty field."
            )
        if not isinstance(raw_next_marker, str) or not raw_next_marker:
            raise ValueError(
                f"{family} {key} leading component-text rule for {raw_field!r} "
                "needs next_record_marker_ascii."
            )
        try:
            field = raw_field.encode("ascii")
            next_marker_name = raw_next_marker.encode("ascii")
        except UnicodeEncodeError as exc:
            raise ValueError(
                f"{family} {key} leading component-text rule must be ASCII."
            ) from exc
        if len(next_marker_name) > 255:
            raise ValueError(
                f"{family} {key} next record marker is too long."
            )
        ...
        if expected_occurrences != 1:
            raise ValueError(
                f"{family} {key} leading component-text rule for {raw_field!r} "
                "must expect exactly one record."
            )
        next_marker = b"\xff" + bytes((len(next_marker_name),)) + next_marker_name
        if not normalized.startswith(reference_marker):
            raise ValueError(
                f"{family} {key} does not begin with its current reference before "
                f"the donor-proven {raw_field!r} record."
            )
        if next_marker not in normalized:
            raise ValueError(
                f"{family} {key} has no next component marker "
                f"{raw_next_marker!r}."
            )
        next_marker_offset = normalized.find(next_marker, len(reference_marker))
        if next_marker_offset <= len(reference_marker):
            raise ValueError(
                f"{family} {key} has no complete leading {raw_field!r} record "
                f"before {raw_next_marker!r}."
            )
        leading_record = normalized[:next_marker_offset]
        field_header = b"\xff\x01Default Font\x00" + field + b"\x00"
        if leading_record.count(field_header) != expected_occurrences:
            raise ValueError(
                f"{family} {key} leading record does not contain exactly one "
                f"donor-proven Default Font {raw_field!r} field."
            )
        if normalized.count(field_header) != expected_occurrences:
            raise ValueError(
                f"{family} {key} has an unexpected duplicate {raw_field!r} field."
            )

        normalized = normalized[next_marker_offset:]
        reports.append(
            {
                "field": raw_field,
                "next_record_marker_ascii": raw_next_marker,
                "removed_occurrences": 1,
                "removed_record_bytes": next_marker_offset,
                "reference_policy": "current_key_at_packet_start",
                "replacement": "remove_complete_leading_record",
            }
        )
    return normalized, reports
...
def _attach_catalogue_terminal_contact_stage(
    *,
    ...
    for group in ordered_groups:
        ...
        data, component_text_record_removals = (
            _remove_catalogue_leading_component_text_records(
                data,
                geometry,
                family=family,
                key=key,
        ...
        family_reports.append(
            {
                ...
                "component_text_record_removals": component_text_record_removals,
...
def attach_catalogue_pin_bidir_terminals_to_project(
    ...
    *,
    ...
    for group_index, group in enumerate(ordered_groups):
        ...
        planning_data, planning_text_record_removals = (
            _remove_catalogue_leading_component_text_records(
                planning_data,
                geometry,
                family=family,
        ...
        if planning_text_record_removals != component_text_record_removals:
            raise ValueError(
                f"{family} {key} component-text record normalization differs between "
