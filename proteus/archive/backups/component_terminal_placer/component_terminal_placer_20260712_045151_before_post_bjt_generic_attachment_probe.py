...
def attach_mixed_component_and_catalogue_bidir_terminals_to_project(
    ...
    *,
    ...
    terminal_leading_catalogue_requested = False
    for family in requested_catalogue:
        profile = catalog.get_profile(family)
        geometry = profile.proteus.get("pin_geometry", {}) if profile is not None else {}
        if (
            isinstance(geometry, dict)
            and geometry.get("clean_packet_attachment_order")
            == "terminal_leading_component_then_wires"
        ):
            terminal_leading_catalogue_requested = True
            break
    ...
    native_component_stream_by_group_id: dict[int, bytes] = {}
    native_component_attachment_records: list[bytes] = []
    ...
    for family in requested_native:
        ...
        if family == "RESISTOR":
            ...
            terminal_leading_catalogue_requested
            and family in GENERIC_TWO_PIN_PROFILES
        ):
            # Generic two-pin units are accepted in their native solo/mixed
            # form, but a terminal-leading BJT final zone cannot follow their
            # inline terminal-first packet.  Preserve the same component link,
            # terminal, and WIRE records while moving this profile into the
            # donor-proven component-stream -> attachment bridge zone.
            for index, (group, wires) in enumerate(
                zip(family_groups, family_wires, strict=True)
            ):
                terminals = tuple(family_terminals[index * 2 : index * 2 + 2])
                if len(terminals) != 2:
                    raise ValueError(
                        f"{family} {getattr(group, 'key', '')} lacks its two "
                        "native terminal records for hybrid bridge emission."
                    )
                native_component_stream_by_group_id[id(group)] = family_patches[
                    id(group)
                ]
                for terminal_record, wire_record in zip(
                    terminals,
                    ...
                    native_component_attachment_records.append(terminal_record)
                    native_component_attachment_records.append(wire_record)
        else:
            for index, (group, wires) in enumerate(
                zip(family_groups, family_wires, strict=True)
            ):
    ...
    for index, group in enumerate(ordered_groups):
        ...
        if group_id in catalogue_component_stream_by_group_id:
            ...
            continue
        if group_id in native_component_stream_by_group_id:
            # Generic two-pin bridge components are serialized with the
            # catalogue component stream, immediately before attachment units.
            continue
    ...
    native_component_stream_records = [
        native_component_stream_by_group_id[id(group)]
        for group in ordered_groups
        if id(group) in native_component_stream_by_group_id
    ]
    ...
    deferred_component_stream_records = (
        native_component_stream_records + catalogue_component_stream_records
    )
    deferred_attachment_records = (
        native_component_attachment_records + catalogue_attachment_records
    )
    if deferred_component_stream_records:
        if not deferred_attachment_records:
            raise ValueError(
                "Hybrid component-stream zone has no attachment units to "
                "terminate its final packet."
            )
        final_deferred_packet = deferred_component_stream_records[-1]
        if len(final_deferred_packet) < 2:
            raise ValueError(
                "Final hybrid component packet is too short for donor-proven "
                "tail normalization."
            )
        ...
        deferred_component_stream_records[-1] = final_deferred_packet[:-1]
        boundary_normalizations += 1
    component_stream_records = local_records + deferred_component_stream_records
    ...
    if catalogue_leading_by_group_id:
        ...
        new_chunk = _append_explicit_single_ff_object_stream_terminator(
            accepted_native_order_stream
            + b"".join(deferred_attachment_records)
        ...
        new_chunk = _ensure_double_ff_object_stream_terminator(
            accepted_native_order_stream + b"".join(deferred_attachment_records)
        )
    new_dsn, _pointers = build_dsn(dsn, dsn, new_chunk)
    ...
    report = {
        ...
        "native_component_stream_component_count": len(
            native_component_stream_by_group_id
        ),
        "native_component_stream_component_keys": [
            _group_key(group)
            for group in ordered_groups
            if id(group) in native_component_stream_by_group_id
        ],
        ...
        "component_record_order_mutation": bool(
            ...
            or native_component_stream_by_group_id
        ),
