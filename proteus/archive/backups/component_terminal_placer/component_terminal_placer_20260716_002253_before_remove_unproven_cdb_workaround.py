...
RIGHT_SIDE_ANGLE = 0
# The user-provided combined `totalmix.pdsprj` donor proves a third active
# terminal/component-pin link class (`03 00`).  It is deliberately accepted
# here rather than coerced to the older solo-route `01 00`/`02 00` classes.
...
def _mixed_catalogue_attachment_order(
    ...
    *,
    family: str,
    key: str,
    allow_subpart_blocks: bool = False,
) -> str:
    ...
    if allow_subpart_blocks:
        allowed.add("subpart_terminal_component_wires")
...
def _combined_totalmix_stream_profile(catalog: Any, stream_mode: str) -> dict[str, Any]:
    """Resolve the donor-proven combined stream profile without a runtime donor.

    `totalmix.pdsprj` is evidence only.  Its normalized stream facts live in
    the component catalogue, so the shared placer still consumes a newly
    placed design and never imports circuit packets or ROOT.CDB from it.
    """

    if stream_mode == "conservative":
        return {}
    if stream_mode != "totalmix_combined_v1":
        raise ValueError(
            "Mixed terminal stream mode must be 'conservative' or "
            f"'totalmix_combined_v1', got {stream_mode!r}."
        )
    raw_profile = {}
    getter = getattr(catalog, "proteus_route_profile", None)
    if callable(getter):
        raw_profile = getter("totalmix_combined_v1")
    if not isinstance(raw_profile, dict) or not raw_profile:
        raise ValueError(
            "Catalogue lacks the required totalmix_combined_v1 route profile."
        )
    ...
    if native_trailer_bytes not in COMPONENT_PIN_LINK_TRAILERS:
        raise ValueError(
            "totalmix_combined_v1 native trailer is not a supported active "
            f"link class: {native_trailer!r}."
        )
    for family, trailer_hex in trailers.items():
        try:
            trailer = bytes.fromhex(str(trailer_hex))
        except ValueError as exc:
            raise ValueError(
                f"totalmix_combined_v1 trailer for {family!r} is not hex."
            ) from exc
        if trailer not in COMPONENT_PIN_LINK_TRAILERS:
            raise ValueError(
                f"totalmix_combined_v1 trailer for {family!r} is unsupported: "
                f"{trailer_hex!r}."
            )
...
def attach_mixed_component_and_catalogue_bidir_terminals_to_project(
    ...
    *,
    ...
    terminal_leading_catalogue_families = {
        ...
        if (
            ...
            and _mixed_catalogue_attachment_order(
                ...
                family=family,
                key="<family-profile>",
                allow_subpart_blocks=is_totalmix_combined,
            )
            in {
                "terminal_leading_component_then_wires",
                "subpart_terminal_component_wires",
    ...
    if (
        has_terminal_leading_catalogue_zone
        and unproven_native_before_terminal_leading
    ...
    if is_totalmix_combined:
        unprofiled = sorted(
            set(requested_catalogue) - set(combined_family_link_trailers)
        )
        if unprofiled:
            raise ValueError(
                "totalmix_combined_v1 has no active link-trailer evidence for "
                f"requested catalogue families: {unprofiled}."
            )
        leading_native_priority = tuple(
            str(item)
            for item in combined_stream_profile["native_leading_family_order"]
        )
    ...
    elif has_terminal_leading_catalogue_zone:
    ...
    # The locked mega leaves the final common-cathode link one byte short of
    # a complete four-byte field.  The user-accepted *combined* display donor
    # proves that it consumes exactly one 00 continuation byte before its
    # local terminal/WIRE units.  The full following locked-mega anode packet
    # is placement infrastructure only: serializing it in this terminalized
    # route makes Proteus discard both displays when it saves the project.
    # Keep the historical CC-only route untouched; this narrow branch is only
    # for the current totalmix route containing a visible anode as well.
    ...
    combined_cathode_footer_sentinel_by_group_id: dict[int, Any] = {}
    combined_cathode_footer_group_ids: set[int] = set()
    combined_cathode_footer_sentinel_ids: set[int] = set()
    has_visible_combined_anode = (
        "7SEG-COM-AN-BLUE" in requested_catalogue
        and any(
            _group_family(group) == "7SEG-COM-AN-BLUE"
            and _group_key(group) != "DISPLAY_ANODE_SENTINEL"
            for group in ordered_groups
        )
    )
        ...
        for group_index, group in enumerate(ordered_groups[:-1]):
            ...
            if (
                ...
                if (
                    ...
                    and has_visible_combined_anode
                    and _group_key(following_group) == "DISPLAY_ANODE_SENTINEL"
                ):
                    combined_cathode_footer_sentinel_by_group_id[id(group)] = (
                        following_group
                    )
                    ...
                    combined_cathode_footer_sentinel_ids.add(id(following_group))
                    continue
    ...
    for family in requested_native:
        ...
        pairs, family_terminals, family_wires, family_patches = (
            _mixed_overlay_family_parts(
                ...
                active_link_trailer=(
                    bytes.fromhex(
                        str(combined_stream_profile["native_active_link_trailer"])
                    )
                    if is_totalmix_combined
                    else b"\x01\x00"
    ...
    for group_index, group in enumerate(ordered_groups):
        family = _group_family(group)
        # The locked-mega cathode packet needs this finalized anode row as a
        # byte-boundary sentinel. It is infrastructure, never a requested
        # visible anode component, and must not receive terminals merely
        # because the request also contains 7SEG-COM-AN-BLUE.
        if _group_key(group) == "DISPLAY_ANODE_SENTINEL":
            continue
        ...
        if (
            ...
            not in set(combined_stream_profile["inline_attachment_orders"])
            | {"component_stream_then_attachment_units"}
        ):
            raise ValueError(
                f"{family} {key} has no totalmix_combined_v1 attachment-order "
                f"evidence for {mixed_attachment_order!r}."
            )
        object_stream_finalizer = str(
            geometry.get("object_stream_finalizer", "double_ff")
        )
        ...
        single_footer_sentinel = combined_cathode_footer_sentinel_by_group_id.get(
            id(group)
        )
        if single_footer_sentinel is not None:
            raw_footer_profile = geometry.get("mixed_combined_cathode_footer")
            if not isinstance(raw_footer_profile, dict):
                raise ValueError(
                    f"{family} {key} lacks the donor-proven combined cathode footer profile."
                )
            raw_footer_hex = raw_footer_profile.get("byte_hex")
            if not isinstance(raw_footer_hex, str):
                raise ValueError(
                    f"{family} {key} combined cathode footer lacks byte_hex."
                )
            try:
                footer = bytes.fromhex(raw_footer_hex)
            except ValueError as exc:
                raise ValueError(
                    f"{family} {key} combined cathode footer is not hexadecimal."
                ) from exc
            if len(footer) != 1:
                raise ValueError(
                    f"{family} {key} combined cathode footer must be exactly one byte."
                )
            expected_raw_packet_size = raw_footer_profile.get("raw_packet_size")
            if (
                not isinstance(expected_raw_packet_size, int)
                or len(original_group_data) != expected_raw_packet_size
            ):
                raise ValueError(
                    f"{family} {key} raw packet size {len(original_group_data)} does not "
                    "match its donor-proven combined footer profile."
                )
            sentinel_data = bytes(getattr(single_footer_sentinel, "data", b""))
            if not sentinel_data.startswith(footer):
                raise ValueError(
                    f"{family} {key} following sentinel does not supply the expected "
                    f"combined footer {footer.hex()}."
                )
            # Preserve the original raw CC end for its end-relative link
            # offsets. The common-cathode field begins three bytes before that
            # boundary and takes this footer as its final 00 trailer byte.
            combined_link_base_end = len(original_group_data)
            original_group_data += footer
            combined_cathode_footer_group_ids.add(id(group))
            combined_infrastructure = {
                "key": _group_key(single_footer_sentinel),
                "family": _group_family(single_footer_sentinel),
                "reason": (
                    "common-cathode display uses exactly the donor-proven one-byte "
                    "continuation after its raw link table; the full locked-mega "
                    "anode sentinel is not serialized"
                ),
                "serialized_footer_hex": footer.hex(),
            }
        elif combined_parent is not None:
        ...
        appended_wire_records: list[bytes] = []
        terminal_count = 0
        combined_link_trailer_hex = (
            str(combined_family_link_trailers[family])
            if is_totalmix_combined
            else None
        )
        for row in mixed_terminal_plans:
            ...
            trailer_hex = (
                ...
                else raw_geometry.get("component_link_trailer")
            )
            if combined_link_base_end is not None and raw_link_offset is not None:
                patched_data, component_link_position, old_suffix, trailer = (
                    _patch_component_link_at_position(
                        patched_data,
                        new_suffix=temporary_suffix,
                        position=combined_link_base_end + int(raw_link_offset),
                        trailer_hex=trailer_hex,
                    )
                )
            else:
        ...
        elif mixed_attachment_order == "subpart_terminal_component_wires":
            if not is_totalmix_combined:
                raise ValueError(
                    f"{family} {key} subpart mixed emission requires an "
                    "explicit donor-proven combined stream profile."
                )
            catalogue_leading_by_group_id[id(group)] = tuple(
                _catalogue_subpart_attachment_blocks(
                    family=family,
    ...
    for index, group in enumerate(ordered_groups):
        ...
        if group_id in combined_catalogue_embedded_group_ids:
            serialized_as_single_footer = (
                group_id in combined_cathode_footer_sentinel_ids
            )
            preserved_rows.append(
                {
                    ...
                    "component_family": family,
                    "reason": (
                        "combined_common_cathode_display_single_footer"
                        if serialized_as_single_footer
                        else "combined_common_cathode_display_sentinel"
                        if group_id in combined_catalogue_infrastructure_group_ids
                        else "combined_common_cathode_visible_anode"
                    ),
                    "byte_preserved": (
                        group_id in combined_catalogue_infrastructure_group_ids
                        and not serialized_as_single_footer
                    ),
                    **(
                        {"serialized_footer_bytes": 1}
                        if serialized_as_single_footer
                        else {}
                    ),
                }
        ...
        if group_id in catalogue_leading_by_group_id:
            if is_totalmix_combined:
                local_records.extend(catalogue_leading_by_group_id[group_id])
        ...
        if group_id in combined_cathode_footer_group_ids:
            if not has_local_component_attachments:
                raise ValueError(
                    "Combined common-cathode footer was planned without its local "
                    "terminal/WIRE attachment units."
                )
            # `data` ends in the one-byte 00 continuation consumed by the
            # common-cathode pin-link. It is not a disposable packet separator.
            emitted = data
        else:
    ...
    if is_totalmix_combined:
        ...
        if object_stream_finalizer == "single_ff":
            ...
            raise ValueError(
                "totalmix_combined_v1 selected unsupported object stream finalizer "
                f"{object_stream_finalizer!r}."
            )
    elif catalogue_leading_by_group_id:
    ...
    source_cdb = read_internal_file(source, "ROOT.CDB")
    preserve_source_cdb_families = tuple(
        sorted(
            family
            for family in requested_catalogue
            if (
                (profile := catalog.get_profile(family)) is not None
                and isinstance(profile.proteus, dict)
                and isinstance(
                    profile.proteus.get("pin_geometry", {}),
                    dict,
                )
    ...
    if preserve_source_cdb_families:
        # The established display profiles explicitly prove their component
        # placer ROOT.CDB as immutable supporting infrastructure.  Rebuilding
        # it from synthetic display keys makes Proteus discard active display
        # bodies on persistence.  Preserve it byte-for-byte; this is a
        # DSN-only terminal attachment, not a CDB mutation.
        write_project_from_parts(source, destination, {"ROOT.DSN": new_dsn})
        cdb_normalization_report = {
            "policy": "source_root_cdb_preserved_by_catalogue_profile",
            "families": list(preserve_source_cdb_families),
            "size_before": len(source_cdb),
        ...
        cdb_keep_packages = sorted(
            {
                ...
                if (
                    _group_key(group)
                    and not _group_key(group).startswith("ANON")
