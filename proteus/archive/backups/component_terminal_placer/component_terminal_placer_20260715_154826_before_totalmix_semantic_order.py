...
def _combined_totalmix_stream_profile(catalog: Any, stream_mode: str) -> dict[str, Any]:
    ...
    component_prefix_order = raw_profile.get("backend_component_prefix_family_order")
    ...
    if not (
        ...
        and isinstance(component_prefix_order, (list, tuple))
        and tuple(str(item) for item in component_prefix_order)
        == ("RESISTOR", "CAP")
...
def attach_mixed_component_and_catalogue_bidir_terminals_to_project(
    ...
    *,
    ...
    if is_totalmix_combined:
        # The accepted all-family donor proves that native R/C attachment units
        # must be emitted before catalogue packets: `C1`, `R001A`, `R001B`,
        # separator, R1, then C1.  This is a backend serialization constraint,
        # not a placement rewrite: each selected component packet, reference,
        # and translated coordinate remains exactly the component placer's.
        # The profile stores families rather than donor slots/IDs so a future
        # placed-design producer remains interchangeable.
        prefix_families = tuple(
            str(item)
            for item in combined_stream_profile[
                "backend_component_prefix_family_order"
            ]
        )
        prefix_groups = [
            group
        ...
        prefix_group_ids = {id(group) for group in prefix_groups}
        ordered_groups = tuple(
            prefix_groups
            + [
                group
                for group in source_component_order
                if id(group) not in prefix_group_ids
            ]
        )
        component_backend_serialization.update(
            {
                "policy": "totalmix_profile_family_prefix",
