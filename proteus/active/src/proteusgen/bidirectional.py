"""Donor-derived bidirectional terminal conversion helpers."""
...
from .pdsprj import read_internal_file
...
ZERO_FIXTURE_ID = "bidirectional_terminal_0_donor"
ONE_EIGHTY_FIXTURE_ID = "bidirectional_terminal_180_donor"
...
def load_templates(zero_project: Path, one_eighty_project: Path) -> BidirTemplates:
    zero_records = extract_bidir_records(_extract_object_chunk(read_internal_file(zero_project, "ROOT.DSN")))
    one_eighty_records = extract_bidir_records(
        _extract_object_chunk(read_internal_file(one_eighty_project, "ROOT.DSN"))
    )
...
def load_production_templates(registry: FixtureRegistry) -> BidirTemplates:
    return load_templates(
        registry.get(ZERO_FIXTURE_ID).path,
        registry.get(ONE_EIGHTY_FIXTURE_ID).path,
    )
...
def convert_production_terminals(
    chunk: bytes,
    registry: FixtureRegistry,
) -> tuple[bytes, list[TerminalReplacement], list[str]]:
    ...
    return converted, replacements, validate_conversion(chunk, converted, replacements)
