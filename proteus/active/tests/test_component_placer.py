...
def test_component_placer_blocks_23x_when_true_donor_count_is_16() -> None:
    donor = TrustedDonor(
        ...
        path=ROOT / "evidence" / "donors" / "manual_downloads_20260615" / "component_placer" / "16x_seq_combo_mega_donor.pdsprj",
        counts={"7490": 16},
        source="test",
    )
...
def test_component_placer_parses_16x_mega_cdb_property_rows() -> None:
    donor = ROOT / "evidence" / "donors" / "manual_downloads_20260615" / "component_placer" / "16x_seq_combo_mega_donor.pdsprj"
...
def test_component_placement_generator_accepts_payload_donor_path(tmp_path: Path) -> None:
    donor = ROOT / "evidence" / "donors" / "manual_downloads_20260618" / "new_component_mega" / "new_components_5x_mega.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(donor),
...
def test_mixed_family_interleave_changes_visual_schedule_not_component_stream(
    ...
    """A mixed stress layout visibly samples every family in each round."""
    ...
    output = tmp_path / "mixed_interleave_no_terminal.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": families,
    ...
    first_round = placements[: len(families)]

    assert result.valid
    assert {row["family"] for row in first_round} == set(families)
    assert all(row["layout_band"] == "combined" for row in placements)
    assert all(row["mixed_family_interleave"] is True for row in placements)
    # Returned component groups retain ROOT.DSN donor order; only their visual
    # slots are round-robin.
    assert [group.family for group in result.selected_groups] != [
        row["family"] for row in placements
    ]
...
def test_totalmix_profile_preserves_placed_packet_order_and_localizes_logic_units(
    ...
    # It must be contiguous with R1: CAP right terminal, both R terminals,
    # donor-proven 00 separator, then R1. The profile emits the full semantic
    # packet phase order while preserving every placed packet's identity and
    # coordinate, preventing native bodies from disappearing behind unrelated
    # terminal blocks in a true IC + non-IC mix.
    output_r1_start = output_chunk.find(b"\xff\x02R1")
    assert output_r1_start >= 0
    terminals_before_r1 = [
        row
        for row in terminal_placer._bidir_label_records(output_chunk)
        if row["start"] < output_r1_start
    ][-3:]
    assert [row["label"] for row in terminals_before_r1] == [
        "C1",
        "R001A",
        "R001B",
    ]
    r1_terminal_end = int(terminals_before_r1[-1]["start"]) + 101 + len("R001B")
    assert output_r1_start == r1_terminal_end + 1
    assert output_chunk[r1_terminal_end] == 0x00
    ...
    u9_report = next(
        row
        for row in report["family_reports"]
        if row.get("component_family") == "4511"
    )
...
def test_totalmix_all_49_keeps_every_component_after_4511_inline_repair(
    ...
    request["donor"] = str(_repo_path(NEW_COMPONENT_MEGA_DONOR))
    request["layout"] = {
        "strategy": "beautify",
        "binary_coordinate_mutation": True,
    }
    base = tmp_path / "totalmix_all_49_base.pdsprj"
    output = tmp_path / "totalmix_all_49_terminalized.pdsprj"
    result = generate_component_placement_project(request, base, full_cdb=True)
    report = attach_mixed_component_and_catalogue_bidir_terminals_to_project(
        base,
        output,
        result.selected_groups,
        native_terminal_families=tuple(request["native_terminal_families"]),
        catalogue_terminal_families=tuple(request["catalogue_terminal_families"]),
        use_donor_terminal_labels=False,
        stream_mode="totalmix_combined_v1",
    )
    base_chunk = _extract_object_chunk(read_internal_file(base, "ROOT.DSN"))
    ...
    base_groups = _raw_groups_from_chunk(base_chunk, _generation_markers())
    output_groups = _raw_groups_from_chunk(
        output_component_chunk,
        _generation_markers(),
    )
    output_stream_groups = {
    ...
    base_counts = Counter(
        group.family for groups in base_groups.values() for group in groups
    )
    output_counts = Counter(
        group.family for groups in output_groups.values() for group in groups
    )
    u9_report = next(
        row
        for row in report["family_reports"]
        if row.get("component_key") == "U9"
    )
    u9_terminals = [
        row
        for row in terminal_placer._bidir_label_records(output_chunk)
        if str(row["label"]).startswith("U9")
    ]

    assert result.valid
    assert report["valid"] is True
    assert len(result.selected_groups) == 49
    assert sum(output_counts.values()) == 49
    assert output_counts == base_counts
    assert report["terminal_count_added"] == 318
    assert report["wire_count_added"] == 318
    assert len(u9_report["terminal_pins"]) == 14
    assert {
        row["component_link_trailer"] for row in u9_report["terminal_pins"]
    } == {"0200"}
    assert len(u9_terminals) == 14
    assert max(
...
def test_hc02_shared_placer_preserves_authoritative_attachment_order_and_paths(
    tmp_path: Path,
) -> None:
    """HC02 preserves its own donor order and full routed input paths."""

    family = "74HC02"
    base = tmp_path / "74HC02_1x_no_terminal.pdsprj"
    output = tmp_path / "74HC02_1x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: 1},
    ...
    profile = load_component_catalog().profile(family)
    geometry = profile.proteus["pin_geometry"]
    expected_order = ("10", "13", "4", "1", "2", "3", "5", "6", "8", "9", "11", "12")
    assert geometry["authoritative_terminal_donor"].endswith(
        "74HC02_user_terminalized_july04.pdsprj"
    )
    assert tuple(geometry["donor_attachment_unit_order"]) == expected_order
    chunk = _extract_object_chunk(read_internal_file(output, "ROOT.DSN"))
    terminal_rows = terminal_placer._bidir_label_records(chunk)
    ...
    assert [row["point_count"] for row in wire_rows] == [2, 2, 2, 2, 2, 2, 3, 3, 2, 2, 2, 2]


@pytest.mark.parametrize("count", (9, 12))
def test_hc02_shared_placer_scales_through_all_complete_locked_packages(
    tmp_path: Path,
    count: int,
) -> None:
    """HC02's independent 9x/12x paths retain all active attachments."""

    family = "74HC02"
    base = tmp_path / f"74HC02_{count}x_no_terminal.pdsprj"
    output = tmp_path / f"74HC02_{count}x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: count},
...
def test_hc08_shared_placer_preserves_authoritative_attachment_contract(
    tmp_path: Path,
) -> None:
    """HC08 emits all twelve donor-ordered grid attachments through shared code."""

    family = "74HC08"
    base = tmp_path / "74HC08_1x_no_terminal.pdsprj"
    output = tmp_path / "74HC08_1x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: 1},
    ...
    profile = load_component_catalog().profile(family)
    geometry = profile.proteus["pin_geometry"]
    expected_order = ("8", "11", "3", "6", "1", "2", "4", "5", "9", "10", "12", "13")
    assert tuple(geometry["donor_attachment_unit_order"]) == expected_order
    chunk = _extract_object_chunk(read_internal_file(output, "ROOT.DSN"))
    terminal_rows = terminal_placer._bidir_label_records(chunk)
    wire_rows = terminal_placer._wire_rows_from_chunk(chunk, chunk_start=0)
...
def test_hc32_shared_placer_preserves_authoritative_attachment_contract(
    tmp_path: Path,
) -> None:
    """HC32 emits its donor-ordered four-gate terminal/WIRE route."""

    family = "74HC32"
    base = tmp_path / "74HC32_1x_no_terminal.pdsprj"
    output = tmp_path / "74HC32_1x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: 1},
...
@pytest.mark.parametrize("count", (9, 15))
def test_hc32_shared_placer_scales_through_locked_mega_capacity(
    tmp_path: Path,
    count: int,
) -> None:
    """HC32's scale routes retain every donor-derived active unit."""

    family = "74HC32"
    base = tmp_path / f"74HC32_{count}x_no_terminal.pdsprj"
    output = tmp_path / f"74HC32_{count}x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: count},
...
@pytest.mark.parametrize("count", (9, 15))
def test_hc86_shared_placer_scales_with_full_routed_wire_contract(
    tmp_path: Path,
    count: int,
) -> None:
    """HC86 scale output retains all restored full WIRE paths."""

    family = "74HC86"
    base = tmp_path / f"74HC86_{count}x_no_terminal.pdsprj"
    output = tmp_path / f"74HC86_{count}x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: count},
...
def test_hc266_shared_placer_matches_authoritative_routed_attachment_contract(
    tmp_path: Path,
) -> None:
    """HC266 retains the user donor's labels, links, and full WIRE paths."""

    family = "74HC266"
    base = tmp_path / "74HC266_1x_no_terminal.pdsprj"
    output = tmp_path / "74HC266_1x_terminalized.pdsprj"
    result = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: 1},
    ...
    profile = load_component_catalog().profile(family)
    geometry = profile.proteus["pin_geometry"]
    expected_order = ("3", "4", "10", "11", "1", "2", "5", "6", "8", "9", "12", "13")
    assert tuple(geometry["donor_attachment_unit_order"]) == expected_order
    assert geometry["authoritative_terminal_donor"].endswith(
        "74HC266_user_terminalized_july04.pdsprj"
    )
    chunk = _extract_object_chunk(read_internal_file(output, "ROOT.DSN"))
    terminal_rows = terminal_placer._bidir_label_records(chunk)
    wire_rows = terminal_placer._wire_rows_from_chunk(chunk, chunk_start=0)
...
def test_hc157_terminal_leading_donor_grammar_has_nonzero_short_wires(
    ...
    """HC157 uses its donor record order but a grid-contact short WIRE."""
    ...
    donor = (
        ...
        / "dil16_mux"
        / family
        / "74HC157_terminalized_primary.pdsprj"
    )
    base = tmp_path / "74HC157_1x_no_terminal.pdsprj"
    output = tmp_path / "74HC157_1x_terminalized.pdsprj"
    placement = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: 1},
    ...
    output_dsn = read_internal_file(output, "ROOT.DSN")
    output_chunk = _extract_object_chunk(output_dsn)
    # The final transformed y coordinate happens to end in FF, followed by
    # the one required structural finalizer byte.
    assert output_chunk.endswith(b"\xff\xff")
    donor_terminals = terminal_placer._bidir_label_records(donor_chunk)
    output_terminals = terminal_placer._bidir_label_records(output_chunk)
    output_wires = terminal_placer._wire_rows_from_chunk(
    ...
    base_group = _raw_groups_from_chunk(base_chunk, (family,))[family][0]
    output_group = _raw_groups_from_chunk(output_chunk, (family,))[family][0]
    first_wire_start = terminal_placer._wire_record_spans(output_chunk)[0][0]
    clean_output = output_group.data[: first_wire_start - output_group.start]
    # The component placer has already removed its raw finalizer byte in the
    # base DSN.  The terminal-leading block must use that same live packet
    # width, rather than reintroducing the stale byte before its native WIREs.
    assert len(clean_output) == len(base_group.data)
    donor_wires = terminal_placer._wire_rows_from_chunk(donor_chunk, chunk_start=0)
    assert output_wires[0]["marker_offset"] == donor_wires[0]["marker_offset"] + 1
    changed_positions = {
        position
        for position, (before, after) in enumerate(
            zip(base_group.data, clean_output)
        )
        if before != after
    }
    expected_positions = {
        position
        for pin in geometry["pins"].values()
        for start in [
            len(base_group.data)
            + int(pin["component_link_offset_from_component_end"])
        ]
        # The fourth byte of the donor-proven `01 00` trailer is already zero
        # in the bare reserved slot, so the byte-level delta is three bytes
        # per active link even though the complete field is four bytes.
        for position in range(start, start + 3)
    }
    assert changed_positions == expected_positions


@pytest.mark.parametrize("count", [9, 15])
def test_hc157_scale_preserves_each_catalogue_attachment_unit(
    tmp_path: Path,
    count: int,
) -> None:
    """Every scaled HC157 packet keeps its own trimmed native attachment unit."""

    family = "74HC157"
    base = tmp_path / f"74HC157_{count}x_no_terminal.pdsprj"
    output = tmp_path / f"74HC157_{count}x_terminalized.pdsprj"
    placement = generate_component_placement_project(
        {
            "donor": str(_repo_path(NEW_COMPONENT_MEGA_DONOR)),
            "components": {family: count},
    ...
    assert report["terminal_suffix_links_valid"] is True
    assert all(
        row["component_placer_finalizer_trimmed"] is True
        and row["terminal_count"] == row["wire_count"] == 14
        for row in report["family_reports"]
    )

    output_dsn = read_internal_file(output, "ROOT.DSN")
    chunk = _extract_object_chunk(output_dsn)
    terminals = terminal_placer._bidir_label_records(chunk)
    wires = terminal_placer._wire_rows_from_chunk(
        chunk,
        chunk_start=terminal_placer._object_chunk_absolute_start(output_dsn),
    ...
    assert len(terminals) == len(wires) == 14 * count
    assert {int(row["suffix"]) for row in terminals} == {
        int(row["suffix"]) for row in wires
    }
    assert all(
        tuple(row["coordinates"][:2]) != tuple(row["coordinates"][2:])
        for row in wires
    )
