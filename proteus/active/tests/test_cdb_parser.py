...
def test_cdb_parser_roundtrips_counter_donors() -> None:
    for name in [
        ...
        roundtrip(ROOT / "evidence" / "donors" / "sequential_counters" / name)
...
def test_cdb_parser_roundtrips_mixed_ic_donors() -> None:
    for name in [
        ...
        roundtrip(ROOT / "evidence" / "donors" / "mixed_ic_analog_batch1" / name)


def test_cdb_parser_preserves_real_row_boundaries() -> None:
    data = read_internal_file(
        ROOT / "evidence" / "donors" / "sequential_counters" / "2_74HC160.pdsprj",
        "ROOT.CDB",
    )
    parsed = parse_cdb(data)
