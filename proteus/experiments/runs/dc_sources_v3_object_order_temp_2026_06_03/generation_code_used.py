"""Temporary DC voltage/current source composition experiment, V3.
...
V3 responds to the V2 user report that all files opened with ISIS.dll errors.
The generated CDB rows now follow the DSN object-emission order: source units
first, then the locked mixed R/C/L group records. Exact donor controls are
included before generated cases to isolate source-donor transplant viability.
"""
...
OUT_ROOT = EXPERIMENT_ROOT / "DC_SOURCES_V3_OBJECT_ORDER_TEST_BATCH"
...
def _build_cdb(rcl_specs: list[rcl.RclSpec], source_specs: list[SourceSpec]) -> bytes:
    # This must match the DSN stream: sources are prepended before RCL groups.
    # Numeric sorting puts V/I IDs after passive IDs and recreates the V2
    # ISIS.dll failure pattern, violating the accepted mixed-RCL CDB rule.
    ordered = [*source_specs, *rcl_specs]
    out = bytearray()
    out += rv9._u32(7)
...
def _generate_case(
    *,
    ...
    input_payload = {
        "case_id": case_id,
        "description": description,
        "source_experiment_schema": "dc-source-rcl-temp/v0.3",
        "sources": [
            {
                "kind": spec.kind,
                "ref": spec.ref,
    ...
    manifest = {
        ...
        "cdb_order_rule": "ROOT.CDB rows are emitted in DSN object order: sources first, then RCL specs",
...
def build_cases() -> list[dict[str, Any]]:
    ...
    dcv_load = DONOR_ROOT / "dc_voltage_03_resistor_load.pdsprj"
    dci_load = DONOR_ROOT / "dc_current_03_resistor_load.pdsprj"
    six = rcl_examples.mixed_rcl_6_case()["groups"]
    twenty_one = rcl_examples.mixed_rcl_21_case()["groups"]
    cases: list[dict[str, Any]] = []
    cases.append(
        _write_repack_case(
            "DCS_V3_T00A_DCV_EXACT_DONOR_REPACK",
            "Exact user DC voltage donor repack. This should behave exactly like dc_voltage_01_default_10v.",
            dcv,
            test_order=1,
        )
    )
    cases.append(
        _write_transplant_control(
            "DCS_V3_T00B_DCV_DONOR_CHUNK_IN_E001",
            "Exact DC voltage donor object chunk and CDB transplanted into E001.",
            base,
            dcv,
            test_order=2,
        )
    ...
    cases.append(
        _write_repack_case(
            "DCS_V3_T00C_DCI_EXACT_DONOR_REPACK",
            "Exact user DC current donor repack. This should behave exactly like dc_current_01_default.",
            dci,
            test_order=3,
        )
    )
    cases.append(
        _write_transplant_control(
            "DCS_V3_T00D_DCI_DONOR_CHUNK_IN_E001",
            "Exact DC current donor object chunk and CDB transplanted into E001.",
            base,
            dci,
            test_order=4,
        )
    ...
    cases.append(
        _write_repack_case(
            "DCS_V3_T00E_DCV_RLOAD_EXACT_DONOR_REPACK",
            "Exact user DC voltage plus resistor-load donor repack.",
            dcv_load,
            test_order=5,
        )
    )
    cases.append(
        _write_transplant_control(
            "DCS_V3_T00F_DCV_RLOAD_DONOR_CHUNK_IN_E001",
            "Exact DC voltage plus resistor-load donor object chunk and CDB transplanted into E001.",
            base,
            dcv_load,
            test_order=6,
        )
    ...
    cases.append(
        _write_repack_case(
            "DCS_V3_T00G_DCI_RLOAD_EXACT_DONOR_REPACK",
            "Exact user DC current plus resistor-load donor repack.",
            dci_load,
            test_order=7,
        )
    )
    cases.append(
        _write_transplant_control(
            "DCS_V3_T00H_DCI_RLOAD_DONOR_CHUNK_IN_E001",
            "Exact DC current plus resistor-load donor object chunk and CDB transplanted into E001.",
            base,
            dci_load,
            test_order=8,
        )
    ...
    cases.append(
        _generate_case(
            case_id="DCS_V3_T01_DCV_6_COMPONENTS",
            ...
            extra_metadata={"test_order": 9, "requested_batch": "6 component DC voltage source"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T02_DCV_21_COMPONENTS",
    ...
    cases.append(
        _generate_case(
            case_id="DCS_V3_T03_DCI_6_COMPONENTS",
            ...
            extra_metadata={"test_order": 11, "requested_batch": "6 component DC current source"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T04_DCI_21_COMPONENTS",
            ...
            sources=[_i(1, "1A", "DI", "I0")],
            extra_metadata={"test_order": 12, "requested_batch": "21 component DC current source"},
        )
    )

    cases.append(
        _generate_case(
            case_id="DCS_V3_T05_COMPLEX_RLC_01",
            ...
            extra_metadata={"test_order": 13, "source_text": "Circuit 1 from pasted request"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T06_COMPLEX_RLC_02",
            ...
            extra_metadata={"test_order": 14, "source_text": "Circuit 2 from pasted request"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T07_COMPLEX_RLC_03",
            ...
            extra_metadata={"test_order": 15, "source_text": "Circuit 3 from pasted request"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T08_COMPLEX_RLC_04",
            ...
            extra_metadata={"test_order": 16, "source_text": "Circuit 4 from pasted request"},
        )
    )
    cases.append(
        _generate_case(
            case_id="DCS_V3_T09_COMPLEX_RLC_05",
            ...
            extra_metadata={"test_order": 17, "source_text": "Circuit 5 from pasted request"},
        )
    )
    return cases
...
def main() -> None:
    ...
    cases = build_cases()
    batch = {
        "batch_id": "DC_SOURCES_V3_OBJECT_ORDER_TEMP_20260603",
        "status": "temp_experimental_not_main",
        "description": "DC voltage/current source donors composed with locked mixed RCL groups, with donor controls and CDB object-order alignment.",
    ...
    archive = shutil.make_archive(str(EXPERIMENT_ROOT / "DC_SOURCES_V3_OBJECT_ORDER_TEMP_2026_06_03"), "zip", OUT_ROOT)
