"""Generate the compact T29 combinational IC legacy-ground patch pack.
...
bidirectional terminal conversion. G0 uses the previously accepted donor
$TERGROUND method because the V2 passive-bidirectional G0 experiment failed
for T29 in user Proteus testing. IC package supply pins are hidden by Proteus
and are not emitted as explicit pin 14 / pin 7 wiring.
"""
...
OUT_ROOT = REPO / "proteus" / "experiments" / "runs" / "ic_final_t29_legacy_ground_v3_temp_2026_06_08"
ARCHIVE_PATH = REPO / "proteus" / "experiments" / "runs" / "IC_FINAL_T29_LEGACY_GROUND_V3_TEMP_2026_06_08.zip"

COMBINED_DEVICE_DONOR = REPO / "proteus" / "active" / "evidence" / "donors" / "combined" / "ALLL_ICS_ALL4_RLC.pdsprj"
...
def build_passive_chunk(case: CircuitCase, first_object_id: int) -> tuple[bytes, list[rcl.RclSpec], list[dict[str, object]], list[dict[str, object]], list[str]]:
    ...
    return (
        ...
        specs,
        topology,
        [item.__dict__ for item in replacements],
        chunk_issues + conversion_issues,
    )
...
def static_issues(
    ...
    if case.passives:
        ...
        expected_ground = 1 if uses_ground else 0
        if object_chunk.count(b"$TERGROUND") != expected_ground:
            issues.append("$TERGROUND count does not match whether the passive topology uses G0")
    else:
...
def write_case(case: CircuitCase) -> dict[str, object]:
    ...
    circuit_input = {
        ...
        "normalizations": {
            ...
            "terminal_policy": "IC signal pins use $TERINPUT/$TEROUTPUT. R/C/L endpoints use $TERBIDIR. Passive G0 uses the previous donor $TERGROUND method.",
            "layout_policy": "Compact T29 V3: IC package spacing is 3,810,000 and nearby passive columns start at x=8,890,000.",
        },
...
def main() -> None:
    ...
    manifests = [write_case(case) for case in cases()[-2:-1]]
    summary = {
        "batch": "IC_FINAL_T29_LEGACY_GROUND_V3_TEMP_2026_06_08",
        "purpose": "Targeted compact-layout T29 retest with the previous donor $TERGROUND method restored.",
        ...
        "shared_rules": [
            ...
            "R/C/L endpoints use $TERBIDIR, but passive G0 keeps the previous donor $TERGROUND method.",
            "The V2 passive bidirectional G0 experiment is rejected for T29 based on user Proteus testing.",
            "This pack emits only T29 because T30 already worked in the V2 pack.",
        ],
        "cases": manifests,
    }
