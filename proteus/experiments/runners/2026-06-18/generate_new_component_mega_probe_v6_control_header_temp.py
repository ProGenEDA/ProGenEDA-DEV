"""Generate V6 control-header probes for SWITCH/POT-HG.

V5 user result:
- FUSE strict338 trim passed; keep-08 failed.
- PNP complete trim passed.
- VPULSE passed.
- M01/M02/M07 passed electrically/opened, but SWITCH/POT-HG controls were
  missing.
- POT-HG trim still had the missing button/control issue.

Rules under test:
- keep the V4/V2 full donor ROOT.CDB policy;
- keep accepted trim rules;
- use the standalone control donors' object-chunk header 00 08 only for
  SWITCH/POT-HG-bearing outputs.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v6_control_header_temp_2026_06_19"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V6_CONTROL_HEADER_TEMP_2026_06_19.zip"
...
def build_cases() -> dict[str, object]:
    ...
    cases.append(copy_raw_case("D00_SWITCH_EXACT_COPY", SWITCH_CONTROL_DONOR, "Exact copy of the user-provided standalone SWITCH donor."))
    cases.append(rebuild_raw_chunk_case("D01_SWITCH_REBUILT_0008", SWITCH_CONTROL_DONOR, "Rebuild standalone SWITCH donor chunk through build_dsn; preserves 00 08 header."))
    cases.append(copy_raw_case("D02_POT_HG_EXACT_COPY", POT_CONTROL_DONOR, "Exact copy of the user-provided standalone POT-HG donor."))
    cases.append(rebuild_raw_chunk_case("D03_POT_HG_REBUILT_0008", POT_CONTROL_DONOR, "Rebuild standalone POT-HG donor chunk through build_dsn; preserves 00 08 header."))

    focused_specs: list[tuple[str, tuple[BodyGroup, ...], str, dict[str, object]]] = []
    for count in (1, 5, 15, 23):
        focused_specs.append(
            (
                f"S{count:02d}_SWITCH_{count:02d}X_HEADER08_TRIM",
                safe_select("SWITCH", count),
                f"{count} SWITCH packets from the updated mega donor with accepted trim and the control donor 00 08 chunk header.",
                {
                    "requested_counts": {"SWITCH": count},
                    "selector": "anonymous_switch_original_order",
                    "final_byte_policy": "legacy_trim",
                    "chunk_prefix": "0008",
                },
            )
        )
        focused_specs.append(
            (
                f"H{count:02d}_POT_HG_{count:02d}X_HEADER08_TRIM",
                safe_select("POT-HG", count),
                f"{count} POT-HG packets from the updated mega donor with accepted trim and the control donor 00 08 chunk header.",
                {
    ...
    mixed_specs = [
        (
            "M01_CONTROL_HEADER_ONE_EACH",
            {family: 1 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 1},
            "V5 M01 rebuilt with accepted family selectors plus 00 08 control header for switch/pot controls.",
        ),
        (
            "M02_CONTROL_HEADER_FIVE_EACH",
            {family: 5 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 5},
            "V5 M02 rebuilt with accepted family selectors plus 00 08 control header for switch/pot controls.",
        ),
        (
            "M07_CONTROL_HEADER_LARGE_STRESS",
            {
                "SWITCH": 15,
                "FUSE": 15,
                "BRIDGE": 15,
            ...
            "V5 M07 rebuilt with accepted family selectors plus 00 08 control header for switch/pot controls.",
        ),
    ]
    for case_id, counts, description in mixed_specs:
        focused_specs.append(
            (
                ...
                description,
                {"requested_counts": counts, "selector": "safe_mixed_cdb_backed", "chunk_prefix": "0008"},
            )
        )
    ...
    return {
        "experiment": "new_component_mega_probe_v6_control_header_temp_2026_06_19",
        "purpose": "Control-header probes for SWITCH/POT-HG missing-button behavior using standalone control donors and updated new-component mega packets.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "Use full donor ROOT.CDB, accepted trim rules, and a 00 08 object chunk header only for SWITCH/POT-HG-bearing outputs. Standalone switch and POT-HG donors both use 00 08, while the broad mega route used 00 00.",
        "donor_counts": {
