"""Generate V7 mixed-control skip probes for SWITCH/POT-HG.

V6 user result:
- all cases opened/worked;
- mixed M01/M02/M07 still displayed the old uncorrect first SWITCH and
  POT-HG controls.

Rules under test:
- keep the V6 accepted 00 08 chunk header;
- keep accepted FUSE/PNP/VPULSE selectors and full donor ROOT.CDB;
- for mixed outputs only, skip the first SWITCH and first POT-HG packet from
  the updated mega donor.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v7_mixed_control_skip_temp_2026_06_19"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V7_MIXED_CONTROL_SKIP_TEMP_2026_06_19.zip"
...
def build_cases() -> dict[str, object]:
    ...
    def fixed_pothg(count: int, *, offset: int = 0) -> tuple[BodyGroup, ...]:
        groups = [
            group
        ...
        if offset:
            groups = groups[offset:]
        if len(groups) < count:
            raise ValueError(f"Need {count} complete POT-HG groups after offset={offset}, found {len(groups)}.")
        return tuple(sorted(groups[:count], key=lambda item: item.start))
    ...
    def mixed_select(family: str, count: int) -> tuple[BodyGroup, ...]:
        if family == "SWITCH":
            return select_family_window(five, family, count, offset=1)
        if family == "POT-HG":
            return fixed_pothg(count, offset=1)
        return safe_select(family, count)

    def mixed_select_many(counts: dict[str, int]) -> tuple[BodyGroup, ...]:
        selected: list[BodyGroup] = []
        for family, count in counts.items():
            selected.extend(mixed_select(family, count))
        return tuple(sorted(selected, key=lambda item: item.start))
    ...
    mixed_specs = [
        (
            "M01_SKIP_FIRST_CONTROL_ONE_EACH",
            {family: 1 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 1},
            "V7 M01 using accepted selectors plus mixed-only skip of first SWITCH/POT-HG packets.",
        ),
        (
            "M02_SKIP_FIRST_CONTROL_FIVE_EACH",
            {family: 5 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 5},
            "V7 M02 using accepted selectors plus mixed-only skip of first SWITCH/POT-HG packets.",
        ),
        (
            "M07_SKIP_FIRST_CONTROL_LARGE_STRESS",
            {
                "SWITCH": 15,
                "FUSE": 15,
                "BRIDGE": 15,
            ...
            "V7 M07 using accepted selectors plus mixed-only skip of first SWITCH/POT-HG packets.",
        ),
    ]
    for case_id, counts, description in mixed_specs:
        focused_specs.append(
            (
                case_id,
                mixed_select_many(counts),
                description,
                {
                    "requested_counts": counts,
                    "selector": "safe_mixed_cdb_backed_skip_first_control_packets",
                    "control_offsets": {"SWITCH": 1, "POT-HG": 1},
                    "chunk_prefix": "0008",
                },
            )
        )
    ...
    return {
        "experiment": "new_component_mega_probe_v7_mixed_control_skip_temp_2026_06_19",
        "purpose": "Mixed-case correction for V6: skip the first SWITCH/POT-HG mega packets while preserving accepted control-header and family selectors.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "For mixed outputs, use full donor ROOT.CDB, accepted trim rules, a 00 08 object chunk header, and offset=1 for SWITCH/POT-HG packet selection so the old first control instance is omitted.",
        "donor_counts": {
