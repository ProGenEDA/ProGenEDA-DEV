"""Generate V5 family-specific repair probes for updated new-component mega failures.

This pack targets the families still failing after V4 user testing:
FUSE, POT-HG, PNP, and the mixed M01/M02/M07 stress selections.

Rules under test:
- keep the V4/V2 full donor ROOT.CDB policy;
- do not change already-passing family selectors;
- avoid partial PNP records by requiring complete 342+ byte PNP packets;
- avoid suspect FUSE rows by taking only 338-byte rows ending in 08;
- test final-byte preservation for control-like FUSE/POT-HG records.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v5_familyfix_temp_2026_06_18"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V5_FAMILYFIX_TEMP_2026_06_18.zip"
...
def write_case(
    ...
    *,
    ...
    if force_chunk is not None:
        ...
        object_chunk, finalization = object_chunk_for(selected)
...
def build_cases() -> dict[str, object]:
    ...
    def fixed_pothg(count: int) -> tuple[BodyGroup, ...]:
        groups = [
            group
        ...
        if len(groups) < count:
            raise ValueError(f"Need {count} complete POT-HG groups, found {len(groups)}.")
        return tuple(sorted(groups[:count], key=lambda item: item.start))
    ...
    for count in (1, 5, 15, 23):
        focused_specs.append(
            (
                f"C{count:02d}_CSOURCE_{count:02d}X_CDB_BACKED_FULLCDB",
                safe_select("CSOURCE", count),
                f"{count} CDB-backed CSOURCE packets from the updated new mega, sorted by natural ref with full donor CDB.",
    ...
    for count in (15, 23):
        focused_specs.append(
            (
                f"V{count:02d}_VPULSE_{count:02d}X_CDB_BACKED_FULLCDB",
    ...
    for count in (15, 23):
        focused_specs.append(
            (
                f"B{count:02d}_BRIDGE_{count:02d}X_SECOND_BLOCK_FULLCDB",
                safe_select("BRIDGE", count),
                f"{count} bridge packets selected after the bad first-block bridge boundary, with full donor CDB.",
                {"requested_counts": {"BRIDGE": count}, "selector": "cdb_backed_offset_14"},
            )
        )

    for count in (1, 5, 15, 23):
        fuse = safe_select("FUSE", count)
        focused_specs.append(
            (
                f"F{count:02d}A_FUSE_{count:02d}X_STRICT338_TRIM",
                fuse,
                f"{count} FUSE packets using only 338-byte rows ending in 08; final byte is trimmed by the legacy terminator rule.",
                {"requested_counts": {"FUSE": count}, "selector": "strict_338_tail08", "final_byte_policy": "legacy_trim"},
            )
        )
        focused_specs.append(
            (
                f"F{count:02d}B_FUSE_{count:02d}X_STRICT338_KEEP08",
                fuse,
                f"{count} FUSE packets using only 338-byte rows ending in 08; final 08 is preserved before FF.",
                {
                    "requested_counts": {"FUSE": count},
                    "selector": "strict_338_tail08",
                    "final_byte_policy": "keep_final_08",
                    "keep_final_byte": True,
                },
            )
        )

    for count in (1, 5, 15, 23):
        pnp = safe_select("PNP", count)
        focused_specs.append(
            (
                f"P{count:02d}A_PNP_{count:02d}X_COMPLETE_TRIM",
                pnp,
                f"{count} PNP packets, requiring complete 342+ byte CDB-backed rows and avoiding the partial Q31-style rows.",
                {"requested_counts": {"PNP": count}, "selector": "complete_cdb_backed_min342", "final_byte_policy": "legacy_trim"},
            )
        )
        focused_specs.append(
            (
                f"P{count:02d}B_PNP_{count:02d}X_COMPLETE_KEEP00",
                pnp,
                f"{count} PNP packets, requiring complete 342+ byte CDB-backed rows and preserving the final 00 before FF.",
                {
                    "requested_counts": {"PNP": count},
                    "selector": "complete_cdb_backed_min342",
                    "final_byte_policy": "keep_final_00",
                    "keep_final_byte": True,
                },
            )
        )
    ...
    for count in (1, 5, 15, 23):
        pothg = safe_select("POT-HG", count)
        focused_specs.append(
            (
                f"H{count:02d}A_POT_HG_{count:02d}X_COMPLETE_TRIM",
                pothg,
                f"{count} POT-HG packets using complete CDB-backed rows; final byte is trimmed by the legacy terminator rule.",
                {"requested_counts": {"POT-HG": count}, "selector": "complete_cdb_backed_tail08", "final_byte_policy": "legacy_trim"},
            )
        )
        focused_specs.append(
            (
                f"H{count:02d}B_POT_HG_{count:02d}X_COMPLETE_KEEP08",
                pothg,
                f"{count} POT-HG packets using complete CDB-backed rows; final 08 is preserved before FF.",
                {
                    "requested_counts": {"POT-HG": count},
                    "selector": "complete_cdb_backed_tail08",
                    "final_byte_policy": "keep_final_08",
                    "keep_final_byte": True,
                },
            )
    ...
    mixed_specs = [
        (
            "M01_FIXED_ONE_EACH_FULLCDB",
            {family: 1 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 1},
            "V2 M01 rebuilt with safe family selectors and full donor CDB.",
        ),
        (
            "M02_FIXED_FIVE_EACH_FULLCDB",
            {family: 5 for family in separated_families if family != "NE555" or finalizable_count(five, family) >= 5},
            "V2 M02 rebuilt with safe family selectors and full donor CDB.",
        ),
        (
            "M07_FIXED_LARGE_STRESS_FULLCDB",
            {
                "SWITCH": 15,
                "FUSE": 15,
                "BRIDGE": 15,
            ...
            "V2 M07 rebuilt with safe selectors, second-block bridge, skipped first fuse, and full donor CDB.",
        ),
    ]
    ...
    for case_id, selected, description, extra in focused_specs:
        ...
        cases.append(write_case(case_id, five, selected, description, keep_final_byte=keep_final_byte, extra=case_extra))

    return {
        "experiment": "new_component_mega_probe_v5_familyfix_temp_2026_06_18",
        "purpose": "Family-specific repair probes for V4 failures using updated new-component mega donors, full donor ROOT.CDB, and isolated selectors for FUSE/POT-HG/PNP.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "Do not prune ROOT.CDB. Select only complete PNP packets, strict good-shape FUSE packets, and complete POT-HG packets. Test whether control-like families require preserving the final 08/00 byte before FF.",
        "donor_counts": {
