"""Generate V12 SWITCH control-spacing probes.

V11 user result:
- exact manual packets still showed the control issue in generated mixed cases;
- moving one SWITCH manually fixed that SWITCH, but the neighboring SWITCH then
  showed the problem.

Rules under test:
- this is a control-hitbox/geometry overlap issue;
- translate only known SWITCH coordinate fields while preserving packet bytes,
  references, CDB, suffixes, and final-record behavior.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v12_control_spacing_temp_2026_06_19"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V12_CONTROL_SPACING_TEMP_2026_06_19.zip"
MANUAL_FIXED_S04 = (
    ROOT
    / "experiments/new_component_mega_probe_v9_control_matrix_temp_2026_06_19"
    / "S04_LARGE_SWITCH_BLOCK2_SHIFT1_POT_BLOCK2"
    / "manualfixedS04_LARGE_SWITCH_BLOCK2_SHIFT1_POT_BLOCK2.pdsprj"
)
...
def extract_manual_family_groups(path: Path, family: str) -> tuple[BodyGroup, ...]:
    dsn = read_internal_file(path, "ROOT.DSN")
    chunk = _extract_object_chunk(dsn)
    starts = object_record_starts(chunk)
    groups: list[BodyGroup] = []
    for index, (start, ref) in enumerate(starts):
        ...
        groups.append(
            BodyGroup(
                ...
                refs=(ref,),
                data=raw,
                source_is_final=(end == len(chunk) - 1),
            )
        )
    return tuple(groups)
...
def translate_switch_packet(data: bytes, *, dx: int = 0, dy: int = 0) -> bytes:
    """Translate only the observed SWITCH coordinate fields.

    The adjacent-control bug follows the visible/control position, not packet
    identity. Keep all refs, suffixes, and non-coordinate bytes unchanged.
    """
...
def build_cases() -> dict[str, object]:
    ...
    manual_switch_groups = extract_manual_family_groups(MANUAL_FIXED_S04, "SWITCH")
    manual_fixed_cdb = read_internal_file(MANUAL_FIXED_S04, "ROOT.CDB")
    separated_families = [
        "VPULSE",
        "VSOURCE",
        "CSOURCE",
    ...
    def control_profile_select_many(
        counts: dict[str, int],
        *,
        switch_offset: int | None = None,
        switch_skip_each_block: int | None = None,
        pothg_offset: int | None = None,
        pothg_skip_each_block: int | None = None,
    ) -> tuple[BodyGroup, ...]:
        selected: list[BodyGroup] = []
        for family, count in counts.items():
            if family == "SWITCH":
                selected.extend(
                    select_family_window(
                        ...
                        family,
                        count,
                        offset=switch_offset or 0,
                        skip_first_each_block=switch_skip_each_block,
                    )
                )
            elif family == "POT-HG":
                selected.extend(
                    fixed_pothg(
                        count,
                        offset=pothg_offset or 0,
                        skip_first_each_block=pothg_skip_each_block,
                    )
                )
            else:
                selected.extend(safe_select(family, count))
    ...
    def manual_switch_select(
        ...
        *,
        ...
        if len(manual_switch_groups) < count:
            raise ValueError(f"Manual fixed S04 has {len(manual_switch_groups)} SWITCH packets, need {count}.")
        selected: list[BodyGroup] = []
        for index, group in enumerate(manual_switch_groups[:count]):
            # Manual S04 stores SWITCH records after all other selected families.
            # Keep them last; the final manual SWITCH record is already in
            # final-record form and must not be followed by another object.
            start = 10_000_000 + (index * 1000)
            dx = base_dx + (index * dx_step)
            dy = base_dy + (index * dy_step)
            selected.append(
                BodyGroup(
                    key=group.key,
                    family=group.family,
                    start=start,
                    end=start + len(group.data),
                    refs=group.refs,
                    data=translate_switch_packet(group.data, dx=dx, dy=dy),
                    source_is_final=(index == count - 1),
                )
            )
        return tuple(selected)

    def manual_switch_select_many(
        counts: dict[str, int],
        *,
        pothg_offset: int,
        switch_base_dx: int = 0,
        switch_base_dy: int = 0,
        switch_dx_step: int = 0,
        switch_dy_step: int = 0,
    ) -> tuple[BodyGroup, ...]:
        selected: list[BodyGroup] = []
        for family, count in counts.items():
            if family == "SWITCH":
                selected.extend(
                    manual_switch_select(
                        count,
                        base_dx=switch_base_dx,
                        base_dy=switch_base_dy,
                        dx_step=switch_dx_step,
                        dy_step=switch_dy_step,
                    )
                )
            elif family == "POT-HG":
                selected.extend(fixed_pothg(count, offset=pothg_offset))
            else:
                selected.extend(safe_select(family, count))
        return tuple(sorted(selected, key=lambda item: item.start))
    ...
    counts_by_base = {case_id: counts for case_id, counts, _description in mixed_specs}
    profile_specs = [
        (
            "T02_ONE_MANUALSW_POT40_DY2540_P08_FULL",
            "M01_CONTROL_ONE_EACH",
            40,
            b"\x00\x08",
            0,
            ...
            0,
            2_540_000,
            "M01: exact manual SWITCH packet with one switch; DY step is inert but records the spacing policy.",
        ),
        (
            "T03_FIVE_MANUALSW_POT40_DY2540_P08_FULL",
            "M02_CONTROL_FIVE_EACH",
            40,
            b"\x00\x08",
            0,
            ...
            0,
            2_540_000,
            "M02: exact manual SWITCH packets, vertical spacing 2,540,000 between control records.",
        ),
        (
            "T04_FIVE_MANUALSW_POT40_DY5080_P08_FULL",
            "M02_CONTROL_FIVE_EACH",
            40,
            b"\x00\x08",
            0,
            ...
            0,
            5_080_000,
            "M02: exact manual SWITCH packets, vertical spacing 5,080,000 between control records.",
        ),
        (
            "T05_FIVE_MANUALSW_POT40_DIAG_P08_FULL",
            "M02_CONTROL_FIVE_EACH",
            40,
            b"\x00\x08",
            0,
            ...
            3_810_000,
            2_540_000,
            "M02: exact manual SWITCH packets, diagonal spacing to separate adjacent control hitboxes.",
        ),
        (
            "T06_LARGE_MANUALSW_POT20_DY2540_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            b"\x00\x08",
            0,
            ...
            0,
            2_540_000,
            "M07: exact manual SWITCH packets and known-good POT-HG block2, vertical spacing 2,540,000.",
        ),
        (
            "T07_LARGE_MANUALSW_POT20_DY5080_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            b"\x00\x08",
            0,
            ...
            0,
            5_080_000,
            "M07: exact manual SWITCH packets and known-good POT-HG block2, vertical spacing 5,080,000.",
        ),
        (
            "T08_LARGE_MANUALSW_POT20_DIAG_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            b"\x00\x08",
            0,
            ...
            3_810_000,
            2_540_000,
            "M07: exact manual SWITCH packets and known-good POT-HG block2, diagonal spacing.",
        ),
        (
            "T09_LARGE_MANUALSW_POT20_BASEDOWN_DY2540_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            b"\x00\x08",
            0,
            ...
            0,
            2_540_000,
            "M07: exact manual SWITCH packets shifted down as a block, then spaced 2,540,000 vertically.",
        ),
    ]
    cases.append(copy_raw_case("T00_MANUAL_FIXED_S04_EXACT_COPY", MANUAL_FIXED_S04, "Exact user-supplied manual-fixed S04 control file."))
    cases.append(rebuild_raw_chunk_case("T01_MANUAL_FIXED_S04_RAW_REBUILD", MANUAL_FIXED_S04, "Rebuild the exact manual-fixed S04 ROOT.DSN chunk and CDB through the packer."))
    ...
    for (
        ...
        counts = counts_by_base[base_case]
        focused_specs.append(
            (
                case_id,
                manual_switch_select_many(
                    counts,
                    pothg_offset=pothg_offset,
                    switch_base_dx=switch_base_dx,
                    switch_base_dy=switch_base_dy,
                    switch_dx_step=switch_dx_step,
                    switch_dy_step=switch_dy_step,
                ),
                description,
                {
                    "requested_counts": counts,
                    "selector": "v12_exact_manual_switch_packets_spaced",
                    "control_offsets": {"SWITCH": "manual_fixed_s04_exact_packets", "POT-HG": pothg_offset},
                    "switch_spacing": {
                        "base_dx": switch_base_dx,
                        "base_dy": switch_base_dy,
                        "dx_step": switch_dx_step,
                        "dy_step": switch_dy_step,
                        "x_offsets": list(SWITCH_X_COORD_OFFSETS),
                        "y_offsets": list(SWITCH_Y_COORD_OFFSETS),
                    },
                    "chunk_prefix": chunk_prefix.hex(),
                    "manual_fixed_cdb": False,
                    "keep_final_byte": False,
                },
            )
        )
    ...
    for case_id, selected, description, extra in focused_specs:
        ...
        prune_cdb = bool(case_extra.pop("prune_cdb", False))
        manual_cdb = bool(case_extra.pop("manual_fixed_cdb", False))
        ...
        if manual_cdb:
            ...
            case_dir.mkdir(parents=True, exist_ok=True)
            output = case_dir / f"{case_id}.pdsprj"
            dsn, pointers = build_dsn(five.dsn, five.dsn, object_chunk)
            write_project_from_parts(
                five.path,
                output,
                {"ROOT.DSN": dsn, "ROOT.CDB": manual_fixed_cdb},
                compression=ZIP_DEFLATED,
            )
            merged = {
                "pointers": pointers,
                "finalization": finalization,
                "cdb_policy": "exact_manual_fixed_s04_cdb",
                "cdb_keep_refs": list(selected_package_refs(selected)),
                **case_extra,
            }
            cases.append(describe_case(case_id, output, five, object_chunk, manual_fixed_cdb, description, merged))
        else:
            cases.append(
                write_case(
                    case_id,
                    ...
                    selected,
                    description,
                    keep_final_byte=keep_final_byte,
    ...
    return {
        "experiment": "new_component_mega_probe_v12_control_spacing_temp_2026_06_19",
        "purpose": "Test SWITCH control-hitbox spacing after user observed moving one switch fixed it but shifted the issue to the next switch.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "Use exact manual-fixed S04 SWITCH records, translate only observed SWITCH coordinate fields, and keep known-good POT-HG/full-CDB context unchanged.",
        "donor_counts": {
        ...
        "source_donors": {
            ...
            "manual_fixed_s04": str(MANUAL_FIXED_S04.relative_to(ROOT)),
