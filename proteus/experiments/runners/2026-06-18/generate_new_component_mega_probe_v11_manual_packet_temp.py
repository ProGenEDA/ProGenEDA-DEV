"""Generate V11 manual-fixed SWITCH packet probes.

V10 user result:
- T07/T08 did not open;
- remaining cases kept the same SWITCH/POT-HG visual issue.

Rules under test:
- use the exact SWITCH packets from the user-provided manual-fixed S04 file;
- test whether exact manual packets are sufficient when reused inside generated
  mixed outputs.
"""
...
OUT_DIR = ROOT / "experiments/new_component_mega_probe_v11_manual_packet_temp_2026_06_19"
ZIP_OUT = ROOT / "experiments/NEW_COMPONENT_MEGA_PROBE_V11_MANUAL_PACKET_TEMP_2026_06_19.zip"
MANUAL_FIXED_S04 = (
    ROOT
...
def build_cases() -> dict[str, object]:
    ...
    def manual_switch_select(count: int) -> tuple[BodyGroup, ...]:
    ...
    def manual_switch_select_many(counts: dict[str, int], *, pothg_offset: int) -> tuple[BodyGroup, ...]:
        selected: list[BodyGroup] = []
        for family, count in counts.items():
            if family == "SWITCH":
                selected.extend(manual_switch_select(count))
            elif family == "POT-HG":
    ...
    profile_specs = [
        (
            "T02_ONE_MANUALSW_POT40_P08_FULL",
            "M01_CONTROL_ONE_EACH",
            40,
            {},
            "M01: use exact manual-fixed S04 SWITCH packet and V9 known-good POT-HG block3.",
            b"\x00\x08",
            False,
            False,
        ),
        (
            "T03_ONE_MANUALSW_POT40_P00_FULL",
            "M01_CONTROL_ONE_EACH",
            40,
            {},
            "M01: same as T02 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
            False,
        ),
        (
            "T04_FIVE_MANUALSW_POT40_P08_FULL",
            "M02_CONTROL_FIVE_EACH",
            40,
            {},
            "M02: use exact manual-fixed S04 SWITCH packets and V9 known-good POT-HG block3.",
            b"\x00\x08",
            False,
            False,
        ),
        (
            "T05_FIVE_MANUALSW_POT40_P00_FULL",
            "M02_CONTROL_FIVE_EACH",
            40,
            {},
            "M02: same as T04 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
            False,
        ),
        (
            "T06_LARGE_MANUALSW_POT20_P08_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            {},
            "M07: use exact manual-fixed S04 SWITCH packets and V9 known-good large POT-HG block2.",
            b"\x00\x08",
            False,
            False,
        ),
        (
            "T07_LARGE_MANUALSW_POT20_P00_FULL",
            "M07_CONTROL_LARGE_STRESS",
            20,
            {},
            "M07: same as T06 but with manual-fixed S04 chunk prefix 00 00.",
            b"\x00\x00",
            False,
            False,
        ),
        (
            "T08_LARGE_MANUALSW_POT20_P00_MANUALCDB",
            "M07_CONTROL_LARGE_STRESS",
            20,
            {},
            "M07: use exact manual-fixed SWITCH packets, POT-HG block2, prefix 00 00, and the exact manual-fixed S04 CDB.",
            b"\x00\x00",
            True,
            True,
        ),
    ...
    for case_id, base_case, pothg_offset, skip_each, description, chunk_prefix, manual_cdb, keep_final_byte in profile_specs:
        counts = counts_by_base[base_case]
        focused_specs.append(
            (
                case_id,
                manual_switch_select_many(counts, pothg_offset=pothg_offset),
                description,
                {
                    "requested_counts": counts,
                    "selector": "v11_exact_manual_switch_packets",
                    "control_offsets": {"SWITCH": "manual_fixed_s04_exact_packets", "POT-HG": pothg_offset},
                    "control_skip_first_each_block": skip_each,
                    "chunk_prefix": chunk_prefix.hex(),
                    "manual_fixed_cdb": manual_cdb,
    ...
    return {
        "experiment": "new_component_mega_probe_v11_manual_packet_temp_2026_06_19",
        "purpose": "Mixed-case correction using exact SWITCH packets extracted from the user-provided manual-fixed S04.",
        "separated_families": separated_families,
        "skipped_requests": skipped,
        "rule_under_test": "Use exact manual-fixed S04 SWITCH records with known-good POT-HG windows; test prefix and CDB context separately.",
        "donor_counts": {
