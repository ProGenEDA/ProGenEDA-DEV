"""Generate repeated-unit mixed R/C/L diagnostics from the 4x donor.

The user-supplied 4x project shows a different scale boundary from V13:

    header, V0 power bridge, repeated complete C/L/R terminal units

Each unit is stored as:

    C output, C input+CAP+wires, L input, R input,
    L ground output+REALIND+wires, R output+RESISTOR+wires

This temporary pack tests that repeated complete unit shape before returning
to arbitrary topologies. Do not promote this into main code until Proteus
open/netlist/simulation feedback accepts the generated scaled cases.
"""
...
def _payload(case_id: str, specs: list[RclSpec], counts: dict[str, Any], topology: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "mixed-rcl-temp/v14-repeated-unit",
        "generator_target": "proteus-8.13-scaled-mixed-rcl-repeated-unit-diagnostic",
        "case_id": case_id,
...
def _write_repack_case(case_id: str, description: str, source_project: Path) -> dict[str, Any]:
    ...
    manifest = {
        "case_id": case_id,
        "status": "temporary_mixed_rcl_v14_control",
...
def _write_generated_case(
    *,
    ...
    manifest = {
        "case_id": case_id,
        "status": "temporary_mixed_rcl_v14_repeated_unit_not_locked",
        "description": description,
...
def main() -> int:
    ...
    cases.append(_write_repack_case("RCL_V14_T00_EXACT_4X_DONOR_REPACK", "Exact deterministic repack of the user-supplied 4x repeated C/L/R donor.", donor))
    cases.append(_write_transplant_control("RCL_V14_T00B_4X_DONOR_CHUNK_IN_E001", "User-supplied 4x donor object chunk and CDB inserted into the clean E001 base.", base, donor))

    generated_defs: list[tuple[str, str, list[tuple[int, int, int]], str]] = [
        ("RCL_V14_T01_1X_UNIT_CONTIGUOUS_IDS", "One repeated C/L/R unit using donor unit order.", _contiguous_global_ids(1), "contiguous"),
        ("RCL_V14_T02_2X_UNIT_CONTIGUOUS_IDS", "Two repeated C/L/R units using donor unit order.", _contiguous_global_ids(2), "contiguous"),
        ("RCL_V14_T03_3X_UNIT_CONTIGUOUS_IDS", "Three repeated C/L/R units using donor unit order.", _contiguous_global_ids(3), "contiguous"),
        ("RCL_V14_T04_4X_UNIT_CONTIGUOUS_IDS", "Four repeated C/L/R units using contiguous IDs.", _contiguous_global_ids(4), "contiguous"),
        ("RCL_V14_T05_4X_UNIT_SUPPLIED_ID_GAPS", "Four repeated C/L/R units using the exact global ID gaps observed in the supplied donor.", _supplied_gap_global_ids(4), "supplied_4x_gaps"),
        ("RCL_V14_T06_6X_UNIT_CONTIGUOUS_IDS", "Six repeated C/L/R units using donor unit order.", _contiguous_global_ids(6), "contiguous"),
        ("RCL_V14_T07_7X_UNIT_CONTIGUOUS_IDS", "Seven repeated C/L/R units using donor unit order.", _contiguous_global_ids(7), "contiguous"),
        ("RCL_V14_T08_21X_UNIT_CONTIGUOUS_IDS", "Twenty-one repeated C/L/R units using donor unit order.", _contiguous_global_ids(21), "contiguous"),
    ]
    ...
    summary = {
        "batch_id": "MIXED_RCL_V14_REPEATED_UNIT_STATIC_20260602",
        "status": "static_generated_awaiting_user_proteus_netlist_test",
        "source_feedback": "V13 single-unit/simple cases worked, but scaled cases with additional R/C/L components failed. User supplied a 4x C/L/R file for more evidence.",
        "method": "Use the supplied 4x donor as the repeated-unit record schema. Emit one V0 power bridge, then complete C/L/R terminal units. Patch native mixed-family resistor records with variable visible-value length so component IDs and suffix links land at the donor-observed offsets.",
